describe ArtistsHelper, type: :helper do
	describe '#initial_link_attributes' do
      ...
      expect(helper.initial_link_attributes('1')).to eq([2, ["1", "1", "selected"], ["2", "2"]])
      expect(helper.initial_link_attributes('2')).to eq([2, ["1", "..."], ["2", "2", "selected"]])
      expect(helper.initial_link_attributes('3')).to eq([3, ["2", "..."], ["3", "3", "selected"]])
      expect(helper.initial_link_attributes('4')).to eq([4, ["3", "..."], ["4", "4", "selected"]])
      expect(helper.initial_link_attributes('5')).to eq([5, ["4", "..."], ["5", "5", "selected"]])
    end
  end
  describe '#li_component' do
    ...
    it 'shoud receive the link_attribute and return the li - second_link' do
      ...
      expect(helper.li_component(["second_link", ["1", "1", "selected"]], "Brazil")).to eq(expected_li)
    end
    it 'shoud receive the link_attribute and return the li - third_link' do
      ...
      expect(helper.li_component(["first_middle_link", ["2", "2"]], "Brazil")).to eq(expected_li)
    end
  end
  ...
  describe '#link_attributes' do
    ...
    it 'shoud receive the summary and a hash with the link_attributes' do
      expect_hash = {
        "first_link" => ["1", "<<"],
        "first_middle_link" => ["200", "200", "selected"],
        "last_link" => ["233602", ">>"],
        "previous_last_link" => ["203", "..."],
        "second_link" => ["199", "..."],
        "second_middle_link" => ["201", "201"],
        "third_middle_link" => ["202", "202"]
      }
